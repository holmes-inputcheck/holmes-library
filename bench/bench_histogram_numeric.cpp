...
// the range length [1, range_len], [range_len + 1, 2 * range_len], ...
int range_len = 1000;
...
typedef struct _data_entry {
	...
	uint64_t derived_group;
	IntFp zk_derived_group;
} data_entry;
...
int main(int argc, char** argv) {
	...
	std::cout << std::endl << "------------ histogram for numeric bench ------------" << std::endl << std::endl;

	{
		FILE * fp = fopen("./benchmark_input.txt", "r");
		fscanf(fp, "%d%d%d", &num_records, &num_groups, &range_len);
		fclose(fp);
	}
	...
	for(int i = 0; i < num_groups; i++) {
		group_start.push_back(1 + i * range_len);
		group_end.push_back(range_len + i * range_len);
	}

	auto group_info = histogram_numeric_init(group_start, group_end);
	...
	for(int i = 0; i < dataset.size(); i++) {
		histogram_numeric_check(dataset[i].inp, dataset[i].zk_inp, group_info, dataset[i].derived_group, dataset[i].zk_derived_group, group_counts, zk_group_counts, zk_zero_checking);
	}

	if(party == ALICE) {
		cout << "after all the group checks, used OT triples = " << ZKFpExec::zk_exec->print_total_triple() << endl;
	}
