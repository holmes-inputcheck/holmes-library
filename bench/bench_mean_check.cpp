...
int main(int argc, char** argv) {
	...
	std::cout << std::endl << "------------ mean check bench ------------" << std::endl << std::endl;
    ...
    mean_check(inp_sum, zk_inp_sum, dataset.size(), mean, zk_mean, fixed_point_shift, zk_zero_checking);

	if (party == ALICE) {
		cout << "after the mean check, used OT triples = " << ZKFpExec::zk_exec->print_total_triple() << endl;
	}
