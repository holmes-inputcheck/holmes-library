...
vector<financial_data_entry> load_dataset(vector<uint64_t> &precomputed_dataset, vector<IntFp> &zk_precomputed_dataset, int num_records) {
	// assert(num_records <= 41188);
	assert(num_records == 411880);

	// vector<vector<uint64_t>> dataset_csv = read_csv("./bench/dataset_1.csv", num_records);
	vector<vector<uint64_t>> dataset_csv = read_csv("./bench/dataset_1.csv", 41188);
	...
	for(int k = 0; k < 10; k++) {
		for(int i = 0; i < dataset_csv.size(); i++) {
			...
			new_entry.inp_age = cur[0];
			new_entry.inp_job = cur[1];
			new_entry.inp_marital = cur[2];
			new_entry.inp_education = cur[3];
			...
			new_entry.zk_age = zk_cur[0];
			new_entry.zk_job = zk_cur[1];
			new_entry.zk_marital = zk_cur[2];
			new_entry.zk_education = zk_cur[3];
			new_entry.zk_default = zk_cur[4];
			new_entry.zk_housing = zk_cur[5];
			new_entry.zk_loan = zk_cur[6];
			new_entry.zk_contact = zk_cur[7];
			new_entry.zk_month = zk_cur[8];
			new_entry.zk_day_of_week = zk_cur[9];
			new_entry.zk_duration = zk_cur[10];
			new_entry.zk_campaign = zk_cur[11];
			new_entry.zk_pdays = zk_cur[12];
			new_entry.zk_previous = zk_cur[13];
			new_entry.zk_poutcome = zk_cur[14];
			new_entry.zk_emp_var_rate = zk_cur[15];
			new_entry.zk_cons_price_idx = zk_cur[16];
			new_entry.zk_cons_conf_idx = zk_cur[17];
			new_entry.zk_euribor3m = zk_cur[18];
			new_entry.zk_nr_employed = zk_cur[19];
			new_entry.zk_y = zk_cur[20];
