...
int main(int argc, char** argv) {
	...
	std::cout << std::endl << "------------ variance check bench ------------" << std::endl << std::endl;
    ...
    uint64_t inp_sum = 0;
    IntFp zk_inp_sum = IntFp(0, PUBLIC);

	for(int i = 0; i < dataset.size(); i++) {
		inp_sum = add_mod(inp_sum, dataset[i].inp);
        zk_inp_sum = zk_inp_sum + dataset[i].zk_inp;
	}

    if(party == ALICE) {
		cout << "after summing the dataset, used OT triples = " << ZKFpExec::zk_exec->print_total_triple() << endl;
	}

    uint64_t inp_mean;
    IntFp zk_inp_mean = IntFp(0, PUBLIC);

    mean_check(inp_sum, zk_inp_sum, dataset.size(), inp_mean, zk_inp_mean, fixed_point_shift, zk_zero_checking);

	if (party == ALICE) {
		cout << "after finding the mean for the dataset, used OT triples = " << ZKFpExec::zk_exec->print_total_triple() << endl;
	}

    uint64_t inp_squared_sum = 0;
    IntFp zk_inp_squared_sum = IntFp(0, PUBLIC);
    for(int i = 0; i < dataset.size(); i++) {
        inp_squared_sum = add_mod(inp_squared_sum, mult_mod(dataset[i].inp, dataset[i].inp));
        zk_inp_squared_sum = zk_inp_squared_sum + dataset[i].zk_inp * dataset[i].zk_inp;
	}

    if (party == ALICE) {
		cout << "after squared summing for the dataset, used OT triples = " << ZKFpExec::zk_exec->print_total_triple() << endl;
	}

    uint64_t variance;
    IntFp zk_variance;
    variance_check(inp_sum, zk_inp_sum, inp_squared_sum, zk_inp_squared_sum, dataset.size(), variance, zk_variance, fixed_point_shift, zk_zero_checking);

    if (party == ALICE) {
		cout << "after variance check for the dataset, used OT triples = " << ZKFpExec::zk_exec->print_total_triple() << endl;
	}
