...
int main(int argc, char** argv) {
	...
	std::cout << std::endl << "------------ histogram for nominal bench ------------" << std::endl << std::endl;

	{
		FILE * fp = fopen("./benchmark_input.txt", "r");
		fscanf(fp, "%d%d", &num_records, &num_groups);
		fclose(fp);
	}
	...
	for(int i = 0; i < dataset.size(); i++) {
		histogram_nominal_check(dataset[i].inp, dataset[i].zk_inp, 1, num_groups, counts, zk_counts, zk_zero_checking);
	}

	if(party == ALICE) {
		cout << "after all the histogram/counting checks, used OT triples = " << ZKFpExec::zk_exec->print_total_triple() << endl;
	}
